...
class morph_compiler
{
    ...
    std::map<l_string, std::map<std::pair<uint, uint>, uint> > prediction_data;
	
	void process_stat_data();
    void add_stats(const l_string& s, uint flex, uint form, uint max_pref_suf_len);
...
#include <turglem/morph_compiler.t>
